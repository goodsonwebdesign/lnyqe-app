...
aws logs create-log-group --log-group-name "/ecs/lynqe-app" || true

# Step 8: Get load balancer subnets
echo "Retrieving Load Balancer info to ensure subnet compatibility..."
ALB_ARN=$(aws elbv2 describe-load-balancers --query "LoadBalancers[?contains(DNSName, 'lynqe')].LoadBalancerArn" --output text)
...
if [ -n "$ALB_ARN" ]; then
  ...
  SUBNET_CONFIG="["
  FIRST=true
  for SUBNET in $(echo $ALB_SUBNETS | jq -r '.[]'); do
    if [ "$FIRST" = true ]; then
  ...
  echo "Could not find ALB with 'lynqe' in name, using default subnets from current service"
  SUBNET_CONFIG="[\"subnet-06f217639afe889bb\", \"subnet-0f56cdbfc696f03a8\"]"
fi

# Step 9: Update the service
echo "Updating ECS service with new task definition and subnets..."
SECURITY_GROUPS=$(aws ecs describe-services --cluster $CLUSTER_NAME --services $SERVICE_NAME --query "services[0].networkConfiguration.awsvpcConfiguration.securityGroups" --output json)
