...
@Component({
  ...
  template: `
    <div class="container mx-auto p-6">
      <h1 class="text-2xl font-bold mb-6">Auth Debugging Tools</h1>
      
      <div class="bg-white dark:bg-neutral-800 p-6 rounded-lg shadow mb-6">
      ...
      <div class="bg-white dark:bg-neutral-800 p-6 rounded-lg shadow mb-6">
        <h2 class="text-xl font-semibold mb-4">Generate API Token</h2>
        <form [formGroup]="tokenForm" class="mb-4">
          <div class="mb-4">
            <label class="block text-sm font-medium mb-1">API Audience</label>
            <input 
              type="text" 
      ...
      <div *ngIf="apiToken" class="bg-white dark:bg-neutral-800 p-6 rounded-lg shadow mb-6">
        <h2 class="text-xl font-semibold mb-4">API Token</h2>
        <div class="mb-4">
          <p class="font-semibold">Token (first 50 chars):</p>
          <p class="break-all bg-gray-100 dark:bg-neutral-700 p-2 rounded">
        ...
        <div class="mb-4">
          ...
          <pre class="bg-gray-100 dark:bg-neutral-700 p-2 rounded overflow-auto max-h-40">{{ decodedHeader }}</pre>
        </div>
        
        <div class="mb-4">
          <p class="font-semibold">Decoded Token Payload:</p>
          <pre class="bg-gray-100 dark:bg-neutral-700 p-2 rounded overflow-auto max-h-60">{{ decodedPayload }}</pre>
        </div>
      ...
      <div *ngIf="tokenError" class="bg-red-100 dark:bg-red-900 p-6 rounded-lg shadow mb-6">
        <h2 class="text-xl font-semibold mb-4 text-red-800 dark:text-red-200">Error Generating Token</h2>
        <pre class="bg-white dark:bg-neutral-800 p-4 rounded overflow-auto max-h-60">{{ tokenError }}</pre>
      </div>
      
      <div class="bg-white dark:bg-neutral-800 p-6 rounded-lg shadow">
        <h2 class="text-xl font-semibold mb-4">Test Your Backend API</h2>
        ...
        <button 
          (click)="testApiCall()" 
          [disabled]="!apiToken"
          class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded disabled:bg-gray-400 disabled:cursor-not-allowed">
          Test API Call
        </button>
        
        <div *ngIf="apiResult" class="mt-4">
          <p class="font-semibold">API Response:</p>
          <pre class="bg-gray-100 dark:bg-neutral-700 p-2 rounded overflow-auto max-h-60">{{ apiResult }}</pre>
        </div>
        
        <div *ngIf="apiError" class="mt-4">
          <p class="font-semibold text-red-600 dark:text-red-400">API Error:</p>
          <pre class="bg-red-50 dark:bg-red-900 p-2 rounded overflow-auto max-h-60">{{ apiError }}</pre>
        </div>
      </div>
    </div>
  `,
